...
<?  // Данные по дню.
    if ($IBLOCK_ID == IBLOCK_MENU_ID && $ID > 0) {
        
        $element = CIBlockElement::getByID($ID)->getNextElement();
        $data = $element->getFields();
        $data['PROPS'] = $element->getProperties();
        
        $section = \CIBlockSection::GetList(
            array(), 
            array('IBLOCK_ID' => IBLOCK_MENU_ID, 'ID' => $data['IBLOCK_SECTION_ID']), 
            false,
            array('ID', 'NAME', 'UF_PROGRAM'), 
            array('nTopCount' => 1)
        )->fetch();
        
        $menulist = new Glyf\Core\Holybean\MenuList(new Glyf\Core\Holybean\Program($section['UF_PROGRAM']));
        $menudays = $menulist->getDayList();
        
        $text = '';
        foreach ($menudays as $i => $menuday) {
            $time = strtotime('+' . ($i + MENU_DAYS_OFFSET) . ' days');
            $wday = Glyf\Core\Helpers\Text::i18nday(date('N', $time), true);
            if ($menuday->getID() == $ID) {
                $text = array('date' => date('d.m.Y', $time), 'day' => $wday);
                break;
            }
        }
        ?>
            <div class="menu-day" style="margin: 10px; font-size: 20px; background: #ffffff; padding: 10px; border-radius: 2px; border: 1px solid #c4ced2;">
                Меню на <?= $text['date'] ?> / <b><?= $text['day'] ?></b>
            </div>
        <?
    }
?>
