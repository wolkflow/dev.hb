...
class ElementHours
{
    ...
    public function GetUserTypeDescription()
    {
        return array(
            'PROPERTY_TYPE'  => 'S',
            'USER_TYPE'      => 'elementhours',
            'DESCRIPTION'    => 'Привязка к элементам с диапазоном часов',
    ...
    public function GetEditField($property, $value, $element)
    {
		...
		$html .= '<tr>';
		$html .= '<td><select name="' . $element['VALUE'] . '[TIME_BEGIN]">';
		for ($i = 0; $i <= 24; $i++) {
			$select = ($value['VALUE']['TIME_BEGIN'] == $i) ? ('selected') : ('');
			$html .= '<option value="' . $i . '" ' . $select . '> ' . str_pad($i, 2, '0', STR_PAD_LEFT) . ' </option>';
		}
		$html .= '</select></td>';
		$html .= '<td><select name="' . $element['VALUE'] . '[TIME_FINISH]">';
		for ($i = 0; $i <= 24; $i++) {
			$select = ($value['VALUE']['TIME_FINISH'] == $i) ? ('selected') : ('');
			$html .= '<option value="' . $i . '" ' . $select . '> ' . str_pad($i, 2, '0', STR_PAD_LEFT) . ' </option>';
		}
		$html .= '</select></td>';
		$html .= '<td>
